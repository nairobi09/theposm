...
namespace thepos
{
    internal class paymentKovan
    {
        ...
        public int requestKovanCardAuth(int tAmount, int tFreeAmount, int tTaxAmount, int tTax, int tServiceAmt, int install, String is_cup, out PaymentCard pCard)
        {
            ...
            String ResCd = Encoding.Default.GetString(rErrCode);
            ...
            if (ResCd == "0000")
            {
                paymentCard.card_no = Encoding.Default.GetString(rCardno);         // 마스킹 카드번호
                paymentCard.tran_serial = Encoding.Default.GetString(rTranSerial); // 거래번호
                paymentCard.install = Encoding.Default.GetString(rHalbu);          // 할부개월
                paymentCard.amount = convert_number(Encoding.Default.GetString(rTamt).Trim()); // 총거래 금액
                paymentCard.tran_date = Encoding.Default.GetString(rTranDate);     // 거래일시
                paymentCard.auth_no = Encoding.Default.GetString(rAuthNo);         // 승인번호
                paymentCard.card_name = Encoding.Default.GetString(rIssueCard);    // 발급사 명
                paymentCard.merchant_no = Encoding.Default.GetString(rMerNo);      // 가맹점 번호
                ...
                String gift_balance = Encoding.Default.GetString(rMsg3);
                gift_balance.Trim();
            ...
            {
                mErrorMsg = Encoding.Default.GetString(rMsg1);
                return -1;
            }
        ...
        public int requestKovanCashAuth(int tAmount, int tFreeAmount, int tTaxAmount, int tTax, int tServiceAmt, String receipt_type, int input_type, String issues_method_no, out PaymentCash pCash)
        {
